...
class WDash extends Component {
  ...
  async componentDidMount() {
    ...
    if (email1 != null) {
      this.setState({ senderId: email1, name: name1 });
    }
  ...
  sendMessage = async () => {
    ...
    alert(sender + " " + receiveId1 + " " + name1 + " " + date1);
    if (receiveId1 !== null) {
      await db
        .collection("messages")
        .add({
  ...
  chatData = (userName, receiveId, senderId) => {
    this.setState({ name: userName, receiveId: senderId });
  };
